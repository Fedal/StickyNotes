...
//  var color = $('#cp2').colorpicker('getValue',);
//}));

$(function () {
  $('#cp2').colorpicker().on('changeColor', function (e) {
    shapeColor = e.color.toHex();
  });
});

var rectBtn = document.getElementById("rectangleBtn");
rectBtn.addEventListener("click", SetRectangle);

var triBtn = document.getElementById("triangleBtn");
triBtn.addEventListener("click", SetTriangle);

var circBtn = document.getElementById("circleBtn");
circBtn.addEventListener("click", SetCircle);
...
function SizeButtons() {
  ...
  var smallBtn = document.getElementById("smallBtn");
  if (shapeSize === 0) {
    smallBtn.className = "btn btn-primary btn-lg btn-block btn-warning";
    mediumBtn.className = "btn btn-primary btn-lg btn-block btn-success";
    bigBtn.className = "btn btn-primary btn-lg btn-block btn-success";
  }
  if (shapeSize === 1) {
    smallBtn.className = "btn btn-primary btn-lg btn-block btn-success";
    mediumBtn.className = "btn btn-primary btn-lg btn-block btn-warning";
    bigBtn.className = "btn btn-primary btn-lg btn-block btn-success";
  }
  if (shapeSize === 2) {
    smallBtn.className = "btn btn-primary btn-lg btn-block btn-success";
...
Triangle.prototype.draw = function (ctx) {
    ...
    ctx.moveTo(this.x, this.y - (this.h / 2)); // give the (x,y) coordinates
...
CanvasState.prototype.draw = function () {
  ...
  if (!this.valid) {
    ...
    if (this.selection != null) {
      ...
      if (this.selection instanceof Rectangle) {
        console.log("I am a rectangle");
        var mySel = this.selection;
        ctx.strokeRect(mySel.x, mySel.y, mySel.w, mySel.h);
      }
      if (this.selection instanceof Triangle) {
        console.log("I am a triangle");
        var mySel = this.selection;
        ctx.beginPath();
        ctx.strokeStyle = "#CC0000";
        ctx.moveTo(mySel.x, mySel.y - (mySel.h / 2)); // give the (x,y) coordinates
        ctx.lineTo(mySel.x + (mySel.w / 2), mySel.y + (mySel.h / 2));
        ctx.lineTo(mySel.x - (mySel.w / 2), mySel.y + (mySel.h / 2));
        ctx.lineTo(mySel.x, mySel.y - (mySel.h / 2));
        ctx.closePath();
      }
      if (this.selection instanceof Circle) {
        console.log("I am a circle");
        var mySel = this.selection;
        ctx.beginPath();
        ctx.arc(mySel.x, mySel.y, mySel.radius, 0, 2 * Math.PI);
        ctx.stroke();
      }
    }
...
//0=rectangle, 1=triangle, 2=cirlce
